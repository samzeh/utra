/**
 * Oslo Temperature Data
 * 
 * Fetches and manages temperature data for Oslo, Norway
 */

import { fetchTemperatureData, fetchTemperatureStats, transformTemperatureToVenue } from '../services/temperatureApi';

const OSLO_COORDINATES = [10.7522, 59.9139];
const LOCATION_NAME = 'Oslo, Norway';

/**
 * Fetch Oslo temperature data and transform to venue format
 */
export async function getOsloTemperatureData(yearRange = null) {
  try {
    ...
    console.error('Error fetching Oslo data:', error);
...
/**
 * Get recent years of Oslo data (default: last 10 years)
 */
export async function getRecentOsloData(years = 10) {
  try {
    ...
    return await getOsloTemperatureData({ start: startYear, end: endYear });
  } catch (error) {
    console.error('Error fetching recent Oslo data:', error);
    return [];
  }
}

/**
 * Get aggregated Oslo data as a single marker
 * Combines recent years into one display point
 */
export async function getOsloAggregatedData(years = 20) {
  try {
    const data = await getRecentOsloData(years);
    ...
    console.error('Error creating aggregated Oslo data:', error);
    return null;
  }
}

/**
 * Get all available Oslo data
 */
export async function getAllOsloData() {
  return await getOsloTemperatureData();
}

/**
 * Simulate data update (for real-time feel)
 */
export function simulateOsloDataUpdate(venue) {
...
export { LOCATION_NAME, OSLO_COORDINATES };
