 ...
 * 
 * Fetches and manages temperature data for Salt Lake City, Utah, USA
 */

import { fetchTemperatureData, fetchTemperatureStats, transformTemperatureToVenue } from '../services/temperatureApi';

const SALT_LAKE_CITY_COORDINATES = [-111.8910, 40.7608];
const LOCATION_NAME = 'Salt Lake City, Utah';

/**
 * Fetch Salt Lake City temperature data and transform to venue format
 */
export async function getSaltLakeCityTemperatureData(yearRange = null) {
  try {
    ...
    console.error('Error fetching Salt Lake City data:', error);
...
/**
 * Get recent years of Salt Lake City data (default: last 10 years)
 */
export async function getRecentSaltLakeCityData(years = 10) {
  try {
    ...
    return await getSaltLakeCityTemperatureData({ start: startYear, end: endYear });
  } catch (error) {
    console.error('Error fetching recent Salt Lake City data:', error);
    return [];
  }
}

/**
 * Get aggregated Salt Lake City data as a single marker
 * Combines recent years into one display point
 */
export async function getSaltLakeCityAggregatedData(years = 20) {
  try {
    const data = await getRecentSaltLakeCityData(years);
    ...
    return {
      id: `salt-lake-city-aggregated`,
      name: `${LOCATION_NAME}`,
      type: 'Climate Station',
      location: LOCATION_NAME,
      coordinates: SALT_LAKE_CITY_COORDINATES,
    ...
    console.error('Error creating aggregated Salt Lake City data:', error);
    return null;
  }
}

/**
 * Get all available Salt Lake City data
 */
export async function getAllSaltLakeCityData() {
  return await getSaltLakeCityTemperatureData();
}

/**
 * Simulate data update (for real-time feel)
 */
export function simulateSaltLakeCityDataUpdate(venue) {
...
export { LOCATION_NAME, SALT_LAKE_CITY_COORDINATES };
